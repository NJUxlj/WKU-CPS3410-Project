"""
Visualization Module for Local Search TSP Experiments
"""
...
from .local_search import LSResult
...
def plot_convergence(result: LSResult, title: str = "LS Convergence Curve",
                     save_path: Optional[str] = None,
                     show_optimal: Optional[float] = None):
    """Plot best-so-far cost across restarts."""
    fig, ax = plt.subplots(figsize=(10, 6))
    iterations = list(range(1, len(result.cost_history) + 1))
    ax.plot(iterations, result.cost_history, 'b-o', markersize=4, linewidth=1.0,
            alpha=0.7, label='Best-so-far cost')
...
def plot_quality_comparison(all_costs: Dict[str, List[float]],
                            ...
                            save_path: Optional[str] = None):
    """Box plot comparing solution quality across configurations."""
    fig, ax = plt.subplots(figsize=(10, 6))
...
def plot_stability_analysis(costs_list: List[float],
                            ...
                            save_path: Optional[str] = None):
    """Histogram + per-run plot to show solution stability."""
...
def generate_all_plots(instance: TSPInstance, result: LSResult,
                       output_dir: str = "results",
                       prefix: str = "ls"):
    """Generate all standard plots for an LS run."""
    output_path = Path(output_dir)
    output_path.mkdir(parents=True, exist_ok=True)

    plot_convergence(
        result,
        title=f"LS Convergence on {instance.name} ({instance.num_cities} cities)",
    ...
    if instance.coordinates is not None:
        plot_tour(
            ...
            title=f"Best Tour Found by LS - {instance.name}",
