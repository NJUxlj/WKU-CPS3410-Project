"""
Visualization Module for Artificial Bee Colony TSP Experiments
"""
...
def plot_convergence(result: ABCResult, title: str = "ABC Convergence Curve",
    ...
    ax.plot(iters, result.mean_cost_history, 'r-', linewidth=0.8, alpha=0.5,
            label='Mean colony cost')
...
def plot_abandoned_sources(result: ABCResult,
                           title: str = "Scout Bee Abandonments",
                           save_path: Optional[str] = None):
    fig, ax = plt.subplots(figsize=(10, 6))
    iters = list(range(len(result.abandoned_sources_history)))
    ax.bar(iters, result.abandoned_sources_history, width=1.0,
           color='orange', alpha=0.7, edgecolor='black', linewidth=0.4)
    ax.set_xlabel('Iteration')
    ax.set_ylabel('Food sources abandoned')
    ax.set_title(title)
...
def plot_stability_analysis(costs_list: List[float],
    ...
    ax2.plot(runs, costs_list, 'o-', color='darkorange', markersize=4,
...
def generate_all_plots(instance: TSPInstance, result: ABCResult,
    ...
    plot_convergence(
        result,
        title=f"ABC Convergence on {instance.name} ({instance.num_cities} cities)",
        save_path=str(output_path / f"{prefix}_convergence.png"),
        show_optimal=instance.optimal_value,
    )
    plot_abandoned_sources(
        result,
        title=f"ABC Scout Bee Activity - {instance.name}",
        save_path=str(output_path / f"{prefix}_abandoned.png"),
    )
    if instance.coordinates is not None:
        plot_tour(
            instance, result.best_tour,
            title=f"Best Tour Found by ABC - {instance.name}",
